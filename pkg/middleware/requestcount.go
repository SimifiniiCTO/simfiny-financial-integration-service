...
func RequestCountUnaryServerInterceptor(e *metrics.MetricsEngine, m *metrics.FinancialIntegrationServiceMetrics) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		...
		e.RecordRequestCountMetric(fmt.Sprintf("%s-%s", method, path), *e.ServiceName, code.Code(opStatusCode))
		return resp, err
	}
}

// RequestCountStreamServerInterceptor Create new stream server interceptor to capture the number of requests.
func RequestCountStreamServerInterceptor(e *metrics.MetricsEngine, m *metrics.FinancialIntegrationServiceMetrics) grpc.StreamServerInterceptor {
