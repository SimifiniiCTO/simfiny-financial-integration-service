...
func TestServer_CreateBankAccount(t *testing.T) {
	type args struct {
		in0 context.Context
		in1 *proto.CreateBankAccountRequest
	}
	tests := []struct {
		name    string
		s       *Server
		args    args
		want    *proto.CreateBankAccountResponse
		wantErr bool
	}{
		// TODO: Add test cases.
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.s.CreateBankAccount(tt.args.in0, tt.args.in1)
			if (err != nil) != tt.wantErr {
				t.Errorf("Server.CreateBankAccount() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Server.CreateBankAccount() = %v, want %v", got, tt.want)
			}
		})
	}
}
