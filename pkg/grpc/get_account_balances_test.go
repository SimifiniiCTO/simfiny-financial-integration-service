...
type GetAccountsBalancesRpcScenario struct {
	scenarioName        string
...
func GetAccountsBalancesRpcScenarios() ([]GetAccountsBalancesRpcScenario, error) {
	resp, err := setupTestPreconditions()
	if err != nil {
		return nil, err
	}

	return []GetAccountsBalancesRpcScenario{
		{
...
func TestGetAccountBalancesRpcOperation(t *testing.T) {
	ctx := context.Background()
	conn := MockGRPCService(ctx)
	defer conn.Close()

	scenarios, err := GetAccountsBalancesRpcScenarios()
	assert.Nil(t, err)
	...
	for _, scenario := range scenarios {
		...
		req := &proto.GetAccountsBalanceRequest{
			VirtualAccountID: scenario.virtualAccountID,
			UserID:           scenario.userID,
		}

		_, err := c.GetAccountsBalances(ctx, req)
		if err != nil {
