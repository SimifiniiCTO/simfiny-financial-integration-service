...
type GetInvestmentsRpcScenario struct {
	scenarioName        string
...
func GetInvestmentsRpcScenarios() ([]GetInvestmentsRpcScenario, error) {
	resp, err := setupTestPreconditions()
	if err != nil {
		return nil, err
	}

	return []GetInvestmentsRpcScenario{
		{
...
func TestGetInvestmentsRpcOperation(t *testing.T) {
	ctx := context.Background()
	conn := MockGRPCService(ctx)
	defer conn.Close()

	scenarios, err := GetInvestmentsRpcScenarios()
	assert.Nil(t, err)
	...
	for _, scenario := range scenarios {
		...
		req := &proto.GetInvestmentsRequest{
			VirtualAccountID: scenario.virtualAccountID,
			UserID:           scenario.userID,
		}

		resp, err := c.GetInvestments(ctx, req)
		if err != nil {
		...
		if !scenario.shouldErrorOccur {
			...
			assert.NotNil(t, resp.Investments)
		}
	}
}
