...
type GetVirtualAccountIDRpcScenario struct {
	scenarioName        string
...
func GetVirtualAccountIDRpcScenarios() ([]GetVirtualAccountIDRpcScenario, error) {
	resp, err := setupTestPreconditions()
	if err != nil {
		return nil, err
	}

	return []GetVirtualAccountIDRpcScenario{
		{
...
func TestGetVirtualAccountIDRpcOperation(t *testing.T) {
	ctx := context.Background()
	conn := MockGRPCService(ctx)
	defer conn.Close()

	scenarios, err := GetVirtualAccountRpcScenarios()
	assert.Nil(t, err)
	...
	for _, scenario := range scenarios {
		...
		req := &proto.GetVirtualAccountIdRequest{
			UserID: scenario.userID,
		}

		resp, err := c.GetVirtualAccountId(ctx, req)
		if err != nil {
		...
		if !scenario.shouldErrorOccur {
			...
			assert.NotNil(t, resp.VirtualAccountID)
		}
	}
}
