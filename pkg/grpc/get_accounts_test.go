...
type GetAccountsRpcScenario struct {
	scenarioName        string
...
func GetAccountsRpcScenarios() ([]GetAccountsRpcScenario, error) {
	resp, err := setupTestPreconditions()
	if err != nil {
		return nil, err
	}

	return []GetAccountsRpcScenario{
		{
...
func TestGetAccountRpcOperation(t *testing.T) {
	ctx := context.Background()
	conn := MockGRPCService(ctx)
	defer conn.Close()

	scenarios, err := GetAccountsRpcScenarios()
	assert.Nil(t, err)
	...
	for _, scenario := range scenarios {
		...
		resp, err := c.GetAccounts(ctx, req)
		if err != nil {
