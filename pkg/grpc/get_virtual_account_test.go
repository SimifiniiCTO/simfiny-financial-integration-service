...
type GetVirtualAccountRpcScenario struct {
	scenarioName        string
...
func GetVirtualAccountRpcScenarios() ([]GetVirtualAccountRpcScenario, error) {
	resp, err := setupTestPreconditions()
	if err != nil {
		return nil, err
	}

	return []GetVirtualAccountRpcScenario{
		{
...
func TestGetVirtualAccountRpcOperation(t *testing.T) {
	...
	for _, scenario := range scenarios {
		...
		resp, err := c.GetVirtualAccount(ctx, req)
		if err != nil {
		...
		if !scenario.shouldErrorOccur {
			...
			assert.NotNil(t, resp.VirtualAccount)
		}
	}
}
