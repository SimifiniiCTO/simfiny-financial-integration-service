...
func (me *MetricsEngine) RecordDatabaseOperationStatusMetric(operation, destination string) {
	me.RecordSummaryMetric(me.Metrics.DatabaseOperationStatusMetric, operation, destination)
}

func (me *MetricsEngine) RecordDatabaseLatencyMetric(operationName string, start time.Time) {
	me.RecordLatencyMetric(me.Metrics.DatabaseOperationLatencyCountMetric, operationName, "", time.Since(start))
}

func (me *MetricsEngine) RecordPlaidOperationStatusMetric(operation, destination string) {
	me.RecordSummaryMetric(me.Metrics.PlaidOperationStatusMetric, operation, destination)
}
