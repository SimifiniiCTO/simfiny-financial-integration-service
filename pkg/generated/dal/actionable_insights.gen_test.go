...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&financial_integration_service_apiv1.ActionableInsightORM{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&financial_integration_service_apiv1.ActionableInsightORM{}) fail: %s", err)
	}
}
...
func Test_actionableInsightORMQuery(t *testing.T) {
	...
	err = _do.Create(&financial_integration_service_apiv1.ActionableInsightORM{})
	if err != nil {
		t.Error("create item in table <actionable_insights> fail:", err)
	}

	err = _do.Save(&financial_integration_service_apiv1.ActionableInsightORM{})
	if err != nil {
		t.Error("create item in table <actionable_insights> fail:", err)
	}

	err = _do.CreateInBatches([]*financial_integration_service_apiv1.ActionableInsightORM{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <actionable_insights> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*financial_integration_service_apiv1.ActionableInsightORM{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&financial_integration_service_apiv1.ActionableInsightORM{}, 0, 1)
	if err != nil {
...
var ActionableInsightORMGetByUserIDTestCase = []TestCase{}

func Test_actionableInsightORM_GetByUserID(t *testing.T) {
	actionableInsightORM := newActionableInsightORM(db)
	do := actionableInsightORM.WithContext(context.Background()).Debug()

	for i, tt := range ActionableInsightORMGetByUserIDTestCase {
